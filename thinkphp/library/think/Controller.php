...
class Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        if(MODULE_NAME == 'home'  && CONTROLLER_NAME == 'Index' && ACTION_NAME == 'index' && isMobile()) {
    ...
    protected function initSaas()
    {
        if (IS_SAAS) {
            $database = Config::get('database');
            $saas = $GLOBALS['SAAS_CONFIG'];
    ...
    protected function checkPrivilege()
    {
        if (IS_SAAS && !SAAS_BASE_USER) {
            $act_list = $GLOBALS['SAAS_CONFIG']['right'][MODULE_NAME];
            $right = Db::name('system_menu')->whereIn("id", $act_list)->cache(true)->getField('right', true);
            $role_right = '';
            foreach ($right as $val) {
                $role_right .= $val.',';
            }
            $role_right = explode(',', $role_right);
            if (!in_array(CONTROLLER_NAME.'@'.ACTION_NAME, $role_right)) {
                if(request()->isAjax() || input('is_ajax') || input('is_json') || strpos(ACTION_NAME,'ajax')!== false){
