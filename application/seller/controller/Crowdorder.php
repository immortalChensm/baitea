...
class Crowdorder extends Base
{
    ...
    public function ajaxindex()
    {
        ...
        //获取众筹的订单商品信息
        $crowd_goods = \think\Db::name("goods")->whereIn("goods_id",$goodsId)->select();
        $crowd_goods_images = [];
        foreach ($crowd_goods as $k=>$v){
            $temp = $v;
            if($temp['crowd_end']-time()>0){
                
                $temp['crowd_end_format'] = count_down($temp['crowd_end']);
                
            }else{
                $temp['crowd_end_format'] = '已结束';
            }
           $temp['crowd_money'] = 
             
            $crowd_goods_images[$v['goods_id']] = $temp;
            
           
        }
        $this->assign("crowdImage",$crowd_goods_images);
        $crowdlist = $this->dealwithcrowd($goods_arr,$orderList);
        $crowdlist_money = [];
        foreach($crowdlist as $k=>$v){
            foreach ($v['orderinfo'] as $money){
                if(($money['order_status']==0||$money['order_status']==1)&&$money['pay_status']==1){
                    $crowdlist_money[$v['goods_id']][] = $money['total_amount'];
                }
                
            } 
        }
        foreach ($crowdlist_money as $k=>$v){
            $crowdlist_money[$k] = (array_sum($v))?:0;
        }
        $this->assign("crowdlist", $crowdlist);
        $this->assign('orderList', $orderList);
        $this->assign('crowdMoney', $crowdlist_money);
    ...
    //处理众筹订单
    private function dealwithcrowd($goods_arr,$orderList)
    {
        $goods = [];
        $order = [];
        $all   = [];
        foreach($goods_arr as $k=>$v){
            $order[$v[0]['goods_id']][] = $v[0]['order_id'];
            $goods[$v[0]['goods_id']]   = $v[0];
        }
        foreach ($orderList as $k=>$v){
            $all[$v['order_id']] = $v;
        }
        foreach ($goods as $k=>$v){
            $orderIdList = $order[$v['goods_id']];
            $temp = [];
            foreach ($orderIdList as $kk=>$orderid){
                $temp[] = $all[$orderid];
            }
            $goods[$k]['orderinfo'] = $temp;
        }
        return $goods;
    }
    // 虚拟商品
    ...
    */
    ...
    //验证当前订单是否达到众筹的条件
    private function is_reachcrowd_condition($order_id)
    {
        //验证一下当前的众筹订单是否
        //众筹目标金额和众筹结止时间是否达到
        //以上条件没有达到一律禁止一系列的操作
        
        //获取当前订单的商品　　一件众筹订单只对应一件商品
        $crowd_order_goods = \think\Db::name("order_goods")->where("order_id",$order_id)->find();
        
        //从订单表获取
        $crowd_order = \think\Db::name("order_goods")->whereIn("goods_id",$crowd_order_goods['goods_id'])->select();
        $crowd_order_id = [];
        foreach ($crowd_order as $k=>$v){
            $crowd_order_id[] = $v['order_id'];
        }
        
        $crowd_order_money = \think\Db::name("order")->whereIn("order_id",$crowd_order_id)->select();
        $crowd_order_money_pay = [];
        foreach ($crowd_order_money as $k=>$v){
            if(($v['order_status']==0||$v['order_status']==1)&&$v['pay_status']==1){
                $crowd_order_money_pay[] = $v['total_amount'];
            }
        }
        
        $crowd_purpose_goods = \think\Db::name("goods")->where("goods_id",$crowd_order_goods['goods_id'])->where("is_crowd_goods",1)->find();
        if(empty($crowd_purpose_goods['goods_id'])){
            //return false;//众筹商品都没有说明是瞎操作
            return [
                "status"=>false,
                "msg"=>"不存在此众筹数据"
            ];
        }
        if($crowd_purpose_goods['crowdfunding_money']>array_sum($crowd_order_money_pay)){
            //return false;
            return [
                "status"=>false,
                "msg"=>"认筹总额未达到众筹目标无法操作",
        ...
        //当前时间是否达到众筹结束时间了
        if($crowd_purpose_goods['crowd_end']>time()){
            //return false;
            return [
                "status"=>false,
                "msg"=>"众筹还没有结束无法操作",
    ...
    {
        $crowd = $this->is_reachcrowd_condition($order_id);
        
        if(!$crowd['status']){
           
            $this->error($crowd['msg'], U('seller/Crowdorder/index'));
        }
        $orderLogic = new OrderLogic();
        $order = $orderLogic->getOrderInfo($order_id);
        if(!$order){
        	$this->error('该订单不存在或没有权利查看', U('seller/Crowdorder/index'));
        }
