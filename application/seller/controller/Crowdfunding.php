...
class Crowdfunding extends Base
{
    ...
    public function addEditGoods()
    {
        ...
        if($goods_id){
            ...
            if(empty($goods)){
                ...
                $goods['crowd_specification'] = unserialize($goods['crowd_specification']);
                $goods['crowd_specification_length'] = count($goods['crowd_specification']['weight']);
                //print_r($goods);
                
                //获取该商品的众筹规格　
                $crowd_attri = \think\Db::name("raise_specification")->where("rid",$goods_id)->select();
                $this->assign("crowdattri",$crowd_attri);
                $this->assign("crowdattri_length",count($crowd_attri));
    ...
    public function save(){
        // 数据验证
        $data =input('post.');
        $data['is_crowd_goods'] = 1;
        /*
        //序列化众筹商品的规则
        $data['crowd_specification'] = serialize([
            "weight"=>$data['weight'],
            'store'=>$data['store'],
            'crowd_price'=>$data['crowd_price']
        ]);
        ...
        unset($data['weight']);
        unset($data['store']);
        unset($data['crowd_price']);
        */
        $data['store_count'] = array_sum($data['store']);
        $weight = $data['weight'];
        $data['cweight'] = $data['weight'];
        
        $data['weight'] = array_sum($weight);
        ...
        $data['shop_price']        = $data['crowdfunding_money'];
        ...
        if ($goods_id > 0) {
            ...
            //验证众筹结束时间必须大于１小时
            $crowd_end = strtotime($this->request->param("crowd_end").":00:00");
           //修改众筹结束时间不得小于原有的时间
            if($crowd_end<$Goods['crowd_end']){
                $this->ajaxReturn([ 'status' => -1, 'msg' => '众筹结束时间不得小于原设置的时间']);
            }
            
            $data['crowd_end'] = strtotime($this->request->param("crowd_end").":00:00");
            ...
            //保存众筹商品的规格
            if(\think\Db::name("raise_specification")->where("rid",input('post.goods_id'))->value("rid")){
            
                foreach ($data['cweight'] as $k=>$v){
                    if(\think\Db::name("raise_specification")->where("rid",input('post.goods_id'))->where("id",$k)->value("rid")){
                        \think\Db::name("raise_specification")->where("rid",input('post.goods_id'))->where("id",$k)->save([
                            "store"=>$data['store'][$k],
                            "weight"=>$v,
                            "price"=>$data['crowd_price'][$k]
                        ]);
                    }else{
                        //run到此说明是添加了新的规格
                        \think\Db::name("raise_specification")->save([
                            "rid"=>$goods_id,
                            "store"=>$data['store'][$k],
                            "weight"=>$v,
                            "price"=>$data['crowd_price'][$k]
            ...
            //验证众筹结束时间必须大于１小时
            $crowd_end = strtotime($this->request->param("crowd_end").":00:00");
            $crowd_diff = $crowd_end-time();
            if($crowd_diff<3600){
                $this->ajaxReturn([ 'status' => -1, 'msg' => '众筹结束时间最少１个小时']);
            }
            ...
            foreach ($data['cweight'] as $k=>$v){
                ...
                $attr_data[] = [
                    'rid'=>$goods_id,
                    'store'=>$data['store'][$k],
                    'weight'=>$data['cweight'][$k],
                    'price'=>$data['crowd_price'][$k]
                ];
                
            } 
          
            \think\Db::name("raise_specification")->insertAll($attr_data);
        }
