...
class Teaorder extends Base {
    ...
    public function ajaxindex(){
        ...
        I('pay_status') != '-1' ? $condition['pay_status'] = I('pay_status') : false;
        I('pay_code') != '' ? $condition['pay_code'] = I('pay_code') : false;
     
        //if($condition['pay_status']==2){
        //    $condition['pay_status']="";
        //}
        
        $count = M('teart_order')->where($condition)->count();
        $Page  = new AjaxPage($count,20);
        $show = $Page->show();
      
        $orderList = M('teart_order')->where($condition)->limit("{$Page->firstRow},{$Page->listRows}")->order("add_time","desc")->fetchSql(false)->select();
        ...
        $userInfo = $this->getuserbyteaorder($usersId);
        $teaInfo = $this->getteabyteaorder($teaId);
        
        foreach ($orderList as $k=>$v){
            $orderList[$k]['username'] = $userInfo[$v['user_id']];
            $orderList[$k]['teainfo'] = $teaInfo[$v['teart_id']];
        }
        //print_r($orderList);
    ...
    //get userinfo by teaorder
    public function getuserbyteaorder($usersId)
    {
        $list = db("users")->field("user_id,mobile,realname")->whereIn("user_id",$usersId)->select();
        $userInfo = [];
        foreach ($list as $k=>$v){
            $userInfo[$v['user_id']] = $v['realname']?:$v['mobile'];
        }
        return $userInfo;
    }
    
    //get teainfo by teaorder
    public function getteabyteaorder($usersId)
    {
        $list = db("tea_art")->field("teart_id,teart_name")->whereIn("teart_id",$usersId)->select();
        $userInfo = [];
        foreach ($list as $k=>$v){
            $userInfo[$v['teart_id']] = $v['teart_name'];
        }
        return $userInfo;
    }
