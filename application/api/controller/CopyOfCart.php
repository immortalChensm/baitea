...
class Cart extends Base {
    ...
    public function cart3()
    {
        ...
        //$shipping_code = ''; //物流计算在此不用
        
        //$user_note = I('user_note'); //卖家留言
        //$coupon_id = I('coupon_id'); //优惠卷id
        ...
        */
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            $result = $orderLogic->addOrder($this->user_id, $address_id, $shipping_code, $invoice_title, $coupon_id, $car_price, '',$user_note,$result['result']['coupon_price'],$user_note); // 添加订单 额外添加的优惠卷$coupon_price
            // 这个人处理完了再减少
    ...
    public function cart4()
    {
        ...
        $this->ajaxReturn(['status'=>1,'msg'=>'获取成功','result' => ['sum_order_amount'=>$sum_order_amount,'goodsinfo'=>$this->getgoodsinfo_bycart4($master_order_sn)]]);
    }    
    
    //订单支付时获取该订单的商品信息
    public function getgoodsinfo_bycart4($order_sn)
    {
        //商品图片
        //商品价格
        //商品购买数量
        ...
        $order = Db::name("Order")->alias("o")
                    ->field("o.order_id,o.coupon_price,o.store_id,s.store_name")
                    ->join("__STORE__ s","s.store_id=o.store_id")
                    ->where("o.master_order_sn",$order_sn)
                    ->find();
        
        $goods = DB::name("OrderGoods")->alias("o")
                    ->field("o.goods_price,o.goods_name,o.goods_num,g.original_img")
                    ->join("__GOODS__ g","o.goods_id=g.goods_id")
