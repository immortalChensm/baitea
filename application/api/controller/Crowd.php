...
class Crowd extends Base {
    ...
    public function goodsList()
    {
        ...
        empty($sort)&&$this->ajaxReturn(['status' =>-1, 'msg' => '参数不可为空']);
        $p     = $this->request->param("p");
        $count = Db::name("goods")->where(["is_crowd_goods"=>1,"is_on_sale"=>1])->count();
        if(!in_array($sort, ['on_time','sales_sum'])){
            $this->ajaxReturn(['status' =>-1, 'msg' => '排序字段错误']);
        }
        ...
        $list  = Db::name("goods")
                    ...
                    ->where(["is_crowd_goods"=>1,"is_on_sale"=>1])
                    ->order($sort?:'on_time')
        ...
        $storeInfo = Db::name("store")->whereIn("store_id",$storeId)->select();
