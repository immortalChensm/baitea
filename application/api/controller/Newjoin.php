...
class Newjoin extends Base
{
    ...
    public function basicInfo()
    {
        if (IS_POST) {
            ...
            $this->apply = Db::name('store_apply')->where(array('user_id' => $this->user_id))->find(); //检查是否申请
            if (empty($this->apply)) {
    ...
    public function remark()
    {
        if (IS_POST) {
            ...
            $post_data['add_time'] = time();  //全部填写完才记录申请时间
    ...
    {
        $this->apply = $apply = Db::name('store_apply')->where(array('user_id' => $this->user_id))->find(); //检查是否申请
        if (!empty($apply)) {
