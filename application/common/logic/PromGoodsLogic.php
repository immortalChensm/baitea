    ...
    public function buyNow($buyGoods){
        if(!$this->checkActivityIsEnd() && $this->checkActivityIsAble()){
            if($buyGoods['goods_num'] > $this->promGoods['buy_limit']){
                return array('status' => 0, 'msg' => '每人限购'.$this->promGoods['buy_limit'].'件', 'result' => '');
            }
        ...
        if($userBuyGoodsNum > $this->promGoods['buy_limit']){
            return array('status' => 0, 'msg' => '每人限购'.$this->promGoods['buy_limit'].'件，您已下单'.$userPromOrderGoodsNum.'件', 'result' => '');
        }
        if($buyGoods['goods_num'] > $residue_buy_limit){  //不算购物车的
            return array('status' => 0, 'msg' => '商品库存不足，你只能购买'.$residue_buy_limit, 'result' => '');
        }
