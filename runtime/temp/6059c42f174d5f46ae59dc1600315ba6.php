...
<div class="ncsc-form-default">
  <form method="post" action="" enctype="multipart/form-data" id="my_store_form">
    ...
    <dl>
      <dt>轮播：</dt>
      <dd>
        <p class="hint">手机店铺页面头部区域下方的轮播图片展示，最多可上传9张图片，推荐图片大小640x240</p>
      </dd>
    </dl>
    <div class="flexslider">
	    <ul class="slides">
	      <?php if(is_array($store_slide) || $store_slide instanceof \think\Collection || $store_slide instanceof \think\Paginator): if( count($store_slide)==0 ) : echo "" ;else: foreach($store_slide as $k=>$vo): ?>
		...
		<?php $__FOR_START_28672__=0;$__FOR_END_28672__=9;for($i=$__FOR_START_28672__;$i < $__FOR_END_28672__;$i+=1){ ?>
	         <li>
		        ...
		        <div class="picture">
				 <img  id="srcpath_<?php echo $i; ?>"  <?php if(empty($store_slide[$i]) || (($store_slide[$i] instanceof \think\Collection || $store_slide[$i] instanceof \think\Paginator ) && $store_slide[$i]->isEmpty())): ?>src="__PUBLIC__/images/not_adv.jpg"<?php else: ?>src="<?php echo $store_slide[$i]; ?>"<?php endif; ?>>
		          <a href="javascript:;" onclick="ClearPicArr3(this);clear_img(<?php echo $i; ?>);" class="del" title="移除">X</a> </div>
		        <div class="url">
					<label>跳转URL...</label>
		          <input type="text" class="text w150" name="store_slide_url[]" value="<?php if(empty($store_slide_url[$i]) || (($store_slide_url[$i] instanceof \think\Collection || $store_slide_url[$i] instanceof \think\Paginator ) && $store_slide_url[$i]->isEmpty())): ?>http://<?php else: ?><?php echo $store_slide_url[$i]; endif; ?>" />
		        </div>
		        <div class="ncsc-upload-btn"> <a href="javascript:;"> <span>
		          <input type="button" hidefocus="true" size="1" class="input-file" onClick="upload_img('<?php echo $i; ?>')" />
		          </span>
...
function upload_img(i){
	turn = i;
	GetUploadify3(1,'mobile_slide','seller','callback');
}
