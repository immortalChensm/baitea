 ...
 <div id="layoutLeft" class="ncsc-layout-left">
  ...
  <div id="layoutRight" class="ncsc-layout-right">
	...
	<table class="ncsc-default-table">
	  ...
	  <tbody>
	  <?php if(empty($collect) || (($collect instanceof \think\Collection || $collect instanceof \think\Paginator ) && $collect->isEmpty())): ?>
	  <tfoot>
	    <tr>
	      <td colspan="20" class="norecord"><div class="warning-option"><i class="icon-warning-sign"></i><span>暂无符合条件的数据记录</span> </div></td>
	    </tr>
	  </tfoot>
	  <?php else: if(is_array($collect) || $collect instanceof \think\Collection || $collect instanceof \think\Paginator): if( count($collect)==0 ) : echo "" ;else: foreach($collect as $k=>$vo): ?>
	     <tr class="bd-line">
		 	<td class="trigger"><input type="checkbox" class="checkitem tc" name="user_id[]" value="<?php echo $vo['user_id']; ?>"/></td>
...
function send_message_all()
{
	...
	var url = "<?php echo U('Seller/User/sendMessage'); ?>";
	if(obj.length > 0){
		var check_val = [];
		obj.each(function(){
